...
public class EncodeUtils {
    ...
    private static final String JFLOW_HEADER = "id=\"FormHtml\"";
    ...
    public static String xssFilter(String text) {
        if (text != null) {
            ...
            //不是html
            boolean isNotHtml = !StringUtil.startsWithIgnoreCase(value, HTML_HEADER);
            //不是xml
            boolean isNotXml = !StringUtil.startsWithIgnoreCase(value, XML_HEADER);
            //不是 JFlow
            boolean isNotJflow = !StringUtil.contains(value, JFLOW_HEADER);
            // 不是JSON Object
            boolean isNotJsonObject = !(StringUtil.startsWith(value, "{") && StringUtil.endsWith(value, "}"));
            // 不是JSON Array
            boolean isNotJsonArray = !(StringUtil.startsWith(value, "[") && StringUtil.endsWith(value, "]"));
            if (isNotHtml
                    && isNotXml
                    && isNotJflow
                    && isNotJsonObject
     ...
     * 预编译SQL过滤正则表达式
     */
    private static Pattern p5 = Pattern.compile("(?:')|(?:--)|(/\\*(?:.|[\\n\\r])*?\\*/)|(\\b(select|update|and|or|delete|insert|trancate|char|into|substr|ascii|declare|exec|count|master|drop|execute)\\b)", Pattern.CASE_INSENSITIVE);
